...
export class AccountingService {
  ...
  public async getCsrActual(query: GetCsrActual): Promise<any> {
    const { endDate, startDate } = query;
...
order by b.name;`,
  ...
  public async getCsrPlan(query: GetCrsPlan): Promise<any> {
    const { endDate, startDate } = query;
...
group by e.name`,
  ...
  public async getPnlActual(query: GetPnlActual): Promise<any> {
    const { endDate, startDate } = query;
    ...
    const queryResult = await this.connectionService.getConnection({
      rawQuery: `select 	z.revenue - y.cogs - x.opex	as actual_pnl
from 
...
-- and b.code not in ('9999998','6230022','6999999','6300021')
  ...
  public async getPnlPlan(query: GetPnlPlan): Promise<any> {
    const { endDate, startDate } = query;
    ...
    const queryResult = await this.connectionService.getConnection({
      rawQuery: `select 	z.plan - y.plan_cogs - x.plan_opex	as plan_pnl
from
...
`,
  ...
  public async getPnlPerUnit(query: GetPnlPlan): Promise<any> {
    const { endDate, startDate } = query;
    ...
    const queryResult = await this.connectionService.getConnection({
      rawQuery: `WITH plan_pnl AS (
    SELECT 
        g.name AS unit,
        SUM(CASE WHEN f.id IN (13, 72, 99, 14) THEN planned_amount ELSE 0 END) AS plan_revenue,
        SUM(CASE WHEN f.id IN (17, 76, 103) THEN planned_amount ELSE 0 END) AS plan_cogs,
        SUM(CASE WHEN f.id = 15 THEN planned_amount ELSE 0 END) AS plan_opex
    FROM crossovered_budget_lines AS a
    JOIN crossovered_budget AS b ON a.crossovered_budget_id = b.id
    JOIN account_budget_post AS c ON a.general_budget_id = c.id
    JOIN account_budget_rel AS d ON c.id = d.budget_id
...
actual_pnl AS (
    SELECT 
        d.name AS unit,
        SUM(CASE WHEN c.id IN (13, 72, 99, 14) THEN balance ELSE 0 END) * -1 AS actual_revenue,
        SUM(CASE WHEN c.id IN (17, 76, 103) THEN balance ELSE 0 END) AS actual_cogs,
        SUM(CASE WHEN c.id = 15 THEN balance ELSE 0 END) AS actual_opex
    FROM account_move_line AS a
    JOIN account_account AS b ON a.account_id = b.id
    JOIN account_account_type AS c ON b.user_type_id = c.id
...
SELECT 
    ...
    COALESCE(p.plan_revenue - p.plan_cogs - p.plan_opex, 0) AS plan_pnl,
    COALESCE(a.actual_revenue - a.actual_cogs - a.actual_opex, 0) AS actual_pnl,
    CASE
        WHEN COALESCE(p.plan_revenue - p.plan_cogs - p.plan_opex, 0) = 0 THEN 0
        ELSE (COALESCE(a.actual_revenue - a.actual_cogs - a.actual_opex, 0) / 
              COALESCE(p.plan_revenue - p.plan_cogs - p.plan_opex, 0)) * 100
    END AS achievement
FROM plan_pnl p
FULL OUTER JOIN actual_pnl a ON p.unit = a.unit
ORDER BY unit;`,
    });
